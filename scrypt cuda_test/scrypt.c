...
int EVP_PBE_scrypt(uint8_t* pass, size_t passlen, uint8_t* salt, size_t saltlen, uint64_t N, uint64_t r, uint64_t p, uint8_t* key, size_t keylen) {
	...
	printf("clock size = %llu\n", result);
	printf("[CPU] Performance of Scrypt/sec = %llu\n", (3700000000) / (result));	//��� �ϰ���� CPU ���ɿ� �°� ���� ���� �ٲ㼭 �����ϼž� ��Ȯ�ؿ�
}
...
int main()
{
	...
	uint32_t r = 1;
	uint32_t p = 1;															//p�� 2�϶�, p�� 4�϶��� ��
	uint32_t N = 1024;
	uint32_t dklen = 32;
	EVP_PBE_scrypt(password, 128, salt, 128, N, r, p, dkout, dklen);
