...
__device__ void scryptBlockMix(uint32_t* B_, uint32_t* B, uint64_t r) // B_의 값에 B의 값을 BlockMix해준 값을 저장해줌
{
	uint64_t i, j;
	uint32_t X[16], * pB;
...
__device__ void scryptROMix(unsigned char* B, uint64_t r, uint64_t N, uint32_t* X, uint32_t* T, uint32_t* V)
{
	...
	for (i = 0; i < N; i++) {
		...
		j = X[16 * (2 * r - 1)] % N;	//mod연산을 해주고
		pV = V + 32 * r * j;			//pV의 값에 V의 j번째 값 넣어주고
		for (k = 0; k < 32 * r; k++)
			T[k] = X[k] ^ *pV++;		//T의 값에 X와 pV의 값 XOR연산
		scryptBlockMix(X, T, r);		//X의 값에 T의 값을 blockMix해서 저장
	}
...
__global__ void GPU_scrypt(uint8_t* B, uint8_t* pass, size_t passlen, uint8_t* salt, size_t saltlen, uint64_t N, uint64_t r, uint64_t p, uint8_t* key, size_t keylen) 
{
	...
	uint64_t total_len;
	Blen = p * 128 * r;													//block 전체의 길이
	Vlen = 32 * r * (N + 2) * sizeof(uint32_t);					//내부에서 사용하는 Vector의 길이
	total_len = Blen + Vlen;											//사용되는 모든 길이의 합
	data_index = (blockDim.x * blockIdx.x) + threadIdx.x;	//현재의 스레드 번호에 대해서
	data_index = data_index * total_len;							//전체 data 길이와 곱해주는 것으로 현재 스레드의 block에 대한 index를 구해줌
	store_index = (blockDim.x * blockIdx.x) + threadIdx.x;	//현재의 스레드 번호에 대해서
	store_index = store_index * keylen;							//전체 key 길이와 곱해주는 것으로 현재 스레드의 key에 대한 index를 구해줌

	uint32_t* X = NULL;												//현재 스레드에서 사용될 X(ROMix함수의 초기 block)의 값을 복사하기 위한 변수
	uint32_t* T = NULL;												//현재 스레드에서 사용될 T(ROMix함수의 중간 결과 block (X XOR V_j 같은 애들))의 값을 복사하기 위한 변수
