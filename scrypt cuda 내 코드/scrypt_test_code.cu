...
//CPU -> GPU
__global__ void GPU_scrypt_second_method(uint8_t* B, uint8_t* pass, size_t passlen, uint8_t* salt, size_t saltlen, uint64_t N, uint64_t r, uint64_t p, uint8_t* key, size_t keylen)
{
...
//GPU -> GPU
__global__ void GPU_scrypt_third_method(uint8_t* B, uint8_t* pass, size_t passlen, uint8_t* salt, size_t saltlen, uint64_t N, uint64_t r, uint64_t p, uint8_t* key, size_t keylen)
{
...
// 현재의 코드는 여러개의 scrypt에서 p의 값이 1이상일 때 p에 대한 병렬 구현을 하기 위한 코드이다.
// 그러므로 blocksize 는 scrypt의 개수 threadsize 는 p의 개수를 나타냄
void performance_test_scrypt_1(uint32_t blocksize, uint32_t threadsize)
{
	...
	printf("scrypt per second is : %4.2f\n", 1000 / elapsed_time_ms * blocksize);
...
void performance_test_scrypt_2(uint32_t blocksize, uint32_t threadsize)
{
	...
	cudaMalloc((void**)&gpu_key, 64 * blocksize);	// PBKDF2를 GPU에서 하지 않기 때문에 사용하지 않아도 됨
	...
	for (int i = 0; i < blocksize; i++)
	{
		...
		cudaMemcpy(gpu_salt + (8 * i), salt, 4, cudaMemcpyHostToDevice);
	...
	GPU_scrypt_second_method << <blocksize, threadsize >> > (gpu_b, gpu_pass, 8, gpu_salt, 4, 1024, 8, threadsize, gpu_key, 64);
...
void performance_test_scrypt_3(uint32_t blocksize, uint32_t threadsize)
{
	...
	GPU_scrypt_third_method << <blocksize, threadsize >> > (gpu_b, gpu_pass, 8, gpu_salt, 4, 1024, 8, threadsize, gpu_key, 64);
